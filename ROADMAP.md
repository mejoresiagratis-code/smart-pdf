...
> en Ajustes y no sustituye el mapeo del asistente), fase 5 en curso (5·4 cerrada en la `0.10.10`;
> quedan la **5·4b** de etiquetado orgánico y la **5·5** de tablas), fases 6 y 7 sin empezar y
> reasignadas a `0.11.0` y `1.0.0`. ⚠️ El HTML se quedó en el estado previo a la `0.10.10`.
...
| **0.10.12** | **Paso 1 — la revisión deja de ser la de Orange**. La tarjeta del contrato enseña el nombre del fichero (`OpenableColumns.DISPLAY_NAME`) en vez del id opaco del URI de SAF (`document:27726`), y ese nombre pasa a ser el título del `FormSchema` que construye el asistente. «Revisar mapeo» abre **el mismo panel que Ajustes › «Analizar y etiquetar un PDF»** (`SchemaReviewPanel`, extraído y compartido por los dos), sembrado con el contrato ya elegido: antes abría `MappingEditor`, que recorre las 21 canónicas de `CANON` sea cual sea el PDF — con el contrato de Aire, 21 destinos de Orange delante de 481 campos propios. Y el botón se ofrece **siempre** que haya un PDF propio con campos: dependía de `needsMapping`, que se apaga en cuanto hay plantilla guardada para esa huella, así que al recargar un PDF ya conocido no había forma de llegar a la revisión. `MappingEditor` no se retira, pero deja de ser alcanzable desde el asistente. ⚠️ Es todo Compose: no se pudo typecheckear en local, el juez es Actions. |
| **0.10.11** | **Procedencia y nombre** (fuera de la numeración de la fase 5: no toca el asistente). Dos arreglos destapados probando el flujo con un juego real de documentación de un alta. `FieldNormalizer` invierte «Apellidos, Nombre» partiendo por la **última** coma: el Modelo 036 escribe el representante con **tres** trozos (`APELLIDO1, APELLIDO2, NOMBRE`, casilla 305) y la regla de la 0.10.8 exigía exactamente dos, así que el valor **salía tal cual al PDF**, con comas y en orden apellidos-primero. Con dos trozos la última coma es la primera, así que Orange no cambia. Y `AutoFillPolicy` gana `ID_DOCS` (DNI, NIE, pasaporte) como fuente dudosa para dirección, teléfono, correos e IBAN: el domicilio del reverso de un DNI es el particular de la persona, no el de la empresa, y **la validación no lo puede detectar** —en el caso probado los dos domicilios estaban en el mismo municipio y provincia, así que `cpProvinciaMsg` daba verde con el equivocado—. Los documentos de identidad siguen siendo fuente fuerte para el nombre y el NIF del representante. ⚠️ Cambio deliberado en Orange: una dirección que salga *sólo* de un documento de identidad pasa de autorrellenarse a pedir confirmación. Verificado con `kotlinc -Werror` y 31 comprobaciones de comportamiento, 0 fallos. |
| **0.10.10** | **Fase 5, tanda 5·4 — el esquema manda**: `FillStep` y `MappingEditor` dejan de recorrer los 21 campos de Orange sea cual sea el PDF y se dibujan desde el `FormSchema` del PDF cargado; `WizardViewModel.chooseUserContract` inspecciona, calcula la huella con el nº de páginas real y construye el esquema si no estaba (arreglando de paso que la huella del asistente y la del editor de etiquetas no casaran nunca); `BuiltinSchemas.recognize(fieldNames)` reconoce Orange y el contrato de Aire; las tres casillas de cabecera del contrato de Aire, que vienen `/Sí` de fábrica, se emiten ALTA=On y las otras dos Off; `FormSchemaBuilder` promociona a checkbox los 12 grupos radio de un solo widget y parte los sueltos por página. Con Orange sale exactamente lo mismo que antes. |
...
- ~~**Tanda de normalización y procedencia**~~ ✅ *hecha en la `0.10.11`*: la inversión
  «Apellidos, Nombre» pasa a partir por la **última** coma (el Modelo 036 escribe el representante
  con tres trozos y la regla anterior, que exigía dos, dejaba el valor intacto y con comas), y los
  documentos de identidad pasan a ser fuente dudosa para dirección, teléfono, correos e IBAN — el
  domicilio del reverso de un DNI es el particular de la persona, y en el caso probado estaba en el
  mismo municipio y provincia que el fiscal, así que la validación CP↔provincia daba verde con el
  equivocado. De los cuatro arreglos que se habían apuntado, dos **no hacían falta**: el firmante
  del 036 ya estaba en `RISKY_SOURCES` y el merge ya deduplica por valor.
